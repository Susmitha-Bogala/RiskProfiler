...
describe('QuestionScreen', () => {
  ...
  it('navigates to the next question if not the last question', async () => {
    const initialState = { risk: { answers: [] } };
    const { getByText } = renderWithProviders(<QuestionScreen />, {
  ...
  it('applies correct styling to selected option', () => {
    const initialState = { risk: { answers: [{ questionId: 1, score: questions[0].options[0].score }] } };
    const { getByTestId } = renderWithProviders(<QuestionScreen />, {
      reduxState: initialState,
    });
